...
(define/contract (make-transform-data-func . args)
  ...
  ;;  Ordinarily this will be called as (lambda (key val) ...) but
  ;;  we'd like these functions to be composable, so let's allow for
  ;;  the case where it's receiving a pair that was generated by an
  ;;  earlier result of make-transform-data-func
  (lambda args
    (define-values (key val)
      (match args
        [(list key val)        (values key val)]
        [(list (cons key val)) (values key val)]))
